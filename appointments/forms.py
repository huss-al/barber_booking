...
class BookingForm(forms.ModelForm):
    cut = forms.ModelChoiceField(
        queryset=CutType.objects.all(), widget=forms.Select(
            attrs={'class': 'form-control'}))
    barber = forms.ModelChoiceField(
        queryset=Barber.objects.all(), widget=forms.Select(
            attrs={'class': 'form-control'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        start_time = datetime.combine(
                self.initial.get('date', datetime.today().date()), time(9, 0))
        end_time = datetime.combine(
                self.initial.get('date', datetime.today().date()), time(18, 0))
        ...
        if appointment_time_choices:
            self.fields['time'].initial = appointment_time_choices[0][0]
